...
def get_prior_course_id(request, current_course_id):
    ...
    if 'prior_course_id' in request.session and 'prior_course_timestamp' in request.session:
        ...
        if dateutil.parser.parse(prior_course_timestamp) + datetime.timedelta(
...
def current_term(request, sort):
    if sort == "best":
        course_type, primary_sort, secondary_sort = ("Best Classes",
                                                     "-quality_score",
                                                     "-difficulty_score")
        ...
        course_type, primary_sort, secondary_sort = ("Layups",
                                                     "-difficulty_score",
                                                     "-quality_score")
    ...
    term_courses = Course.objects.for_term(constants.CURRENT_TERM,
                                           dist).prefetch_related(
                                               'distribs', 'review_set',
                                               'courseoffering_set').order_by(
                                                   primary_sort,
                                                   secondary_sort)
    ...
    return render(
        request, 'current_term.html', {
            ...
            courses_and_votes,
            'distribs':
            DistributiveRequirement.objects.all(),
            'page_javascript':
            'LayupList.Web.CurrentTerm({})'.format(for_layups_js_boolean)
        })
...
def course_detail(request, course_id):
    ...
    paginator = Paginator(course.review_set.all().order_by("-term"),
                          LIMITS["reviews"])
    try:
    ...
    similarity_recommendations = course.recommendations.filter(
        creator=Recommendation.DOCUMENT_SIMILARITY, ).order_by(
            '-weight').prefetch_related('recommendation')

    # Get instructor names based on review
    professors_and_review_count = list(
        course.review_set.values("professor").annotate(
            Count("professor")).order_by("-professor__count").values_list(
                "professor", "professor__count"))

    # Get instructor names based on instrutors parsed from course offerings
    professors_and_review_count += [
        (instructor_name, 0) for instructor_name in Instructor.objects.filter(
            courseoffering__course=course, ).exclude(name__in=[
    ...
    return render(
        request, 'course_detail.html', {
            ...
            'review_form': form,
            'page_javascript':
            'LayupList.Web.CourseDetail({})'.format(course_id)
        })
...
def departments(request):
    ...
    department_codes_and_counts = (
        Course.objects.values('department').annotate(
            Count('department')).order_by('department').values_list(
                'department', 'department__count'))
    return render(
        request, 'departments.html', {
            'departments': [(code, get_department_name(code), count)
...
def course_search(request):
    ...
    if len(query) not in Course.objects.DEPARTMENT_LENGTHS:
        courses = sorted(courses,
                         key=lambda c: c.review_set.count(),
                         reverse=True)

    return render(
        request, 'course_search.html', {
            'term': constants.CURRENT_TERM,
            'query': query,
...
def course_review_search(request, course_id):
    ...
    return render(
        request, 'course_review_search.html', {
            'query': query,
            'course': course,
...
def medians(request, course_id):
    ...
    for course_median in CourseMedian.objects.filter(course=course_id):
        ...
        medians_by_term[course_median.term].append({
            'median':
            course_median.median,
            'enrollment':
            course_median.enrollment,
            'section':
            course_median.section,
            'numeric_value':
            numeric_value_for_grade(course_median.median)
        })
    ...
    return JsonResponse({
        ...
        sorted(
            [{
                ...
                'avg_numeric_value':
                sum(m['numeric_value']
                    for m in term_medians) / len(term_medians),
                'courses':
                term_medians,
...
def course_professors(request, course_id):
    return JsonResponse({
        ...
        sorted(
            set(
                Review.objects.filter(course=course_id).values_list(
                    'professor', flat=True).distinct())
            | set(
                Instructor.objects.filter(courseoffering__course=course_id, ).
                values_list("name", flat=True).distinct()))
    })
...
def vote(request, course_id):
    ...
    category = (Vote.CATEGORIES.DIFFICULTY
                if forLayup else Vote.CATEGORIES.QUALITY)
    new_score, is_unvote = Vote.objects.vote(int(value), course_id, category,
                                             request.user)
