    ...
    try:
        ...
        for submission in submissions:
            ...
            for key in r.keys("session:*"):
                ...
                if data.get("status") == "turnstile_verified" and str(data.get("code")).strip() == code.strip():
                    new_data = {
                        "code": code,
        ...
        if matched_session_id:
            return JsonResponse({"success": True, "code": code, "session_id": matched_session_id})
        else:
            return JsonResponse({"success": False, "error": "未找到匹配的会话"}, status=404)
...
def complete_login(request):
    ...
    if not session_id:
        return JsonResponse({"status": "error", "detail": "Missing session_id"}, status=400)
    ...
    if not value:
        return JsonResponse({"status": "error", "detail": "Session not found or expired"}, status=404)
        
    try:
        session_data = json.loads(value)
    except json.JSONDecodeError:
        return JsonResponse({"status": "error", "detail": "Invalid session data"}, status=500)
        
    if session_data.get("status") != "fully_verified":
        return JsonResponse({"status": "error", "detail": "Session not fully verified"}, status=400)
        
    student_id = session_data.get("student_id")
    name = session_data.get("name")
    
    if not student_id or not name:
        return JsonResponse({"status": "error", "detail": "Missing student_id or name in session"}, status=500)
